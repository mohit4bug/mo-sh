...
func NewRouter(db *sqlx.DB, redisClient *redis.Client, ctx context.Context) *gin.Engine {
	...
	v1 := r.Group("/api/v1")
	{
		...
		v1.GET("/servers/:serverID/pending-logs", middlewares.Auth(redisClient, ctx), serverRepository.GetPendingLogs)
